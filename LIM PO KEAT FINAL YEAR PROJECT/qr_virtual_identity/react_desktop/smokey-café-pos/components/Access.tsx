...
import { ScanLine, Shield, Lightbulb, User, Smartphone } from 'lucide-react';
    ...
    >
      ...
      <motion.div variants={itemVariants} className="text-center flex flex-col items-center gap-6 mb-12">
        ...
        >
          ...
          <span>Logged in as: <span className="font-bold">sp006@apu.edu.my</span></span>
        </motion.div>
      </motion.div>

      {/* Access Control Trigger */}
      <div className="w-full max-w-2xl flex flex-col gap-6 mb-12">
        <h2 className="text-xl font-bold text-gray-800 text-center mb-2">Access Control</h2>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          transition={ANIMATION_SPRING}
          className="group w-full bg-[#F97316] hover:bg-[#EA580C] text-white rounded-[2rem] p-6 flex items-center justify-center gap-4 shadow-lg shadow-orange-200 transition-all border-b-4 border-[#C2410C] active:border-b-0 active:translate-y-1 relative overflow-hidden"
        >
            <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity" />
            <Shield size={32} strokeWidth={2.5} />
            <span className="text-2xl font-bold">Access Control Mode</span>
        </motion.button>
      </div>
